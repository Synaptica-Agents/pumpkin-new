# -*- coding: utf-8 -*-
"""Baut Creativity-Bewertungstest.xlsx aus results.json (Vorbild: charts-eval).
Optionales Argument: Ausgabedateiname (Default Creativity-Bewertungstest.xlsx)."""
import json, os, sys
...
headers = ["Schwierigkeit", "Kategorie", "Frage", "Referenz-Ideen", "Input (an die KI)",
           "Erwartete Qualität", "KI-Feedback", "Score (0-100)",
           "Struktur /40", "Inhalt /50", "Kreativität /10", "Flagged"]
ws.append(headers)
...
for r in rows:
    ws.append([r["difficulty"], r["category"], r["prompt"], r["muster"], r["input"],
               r["tier"], r["feedback"], r["score"], r["s_structure"], r["s_content"],
               r["s_creativity"], "ja" if r["flagged"] else ""])
...
widths = [12, 16, 36, 42, 42, 13, 46, 10, 11, 11, 11, 8]
...
def add_summary(title, subset):
    ...
    ws2.append(["Qualität", "n", "Ø Score", "Ø Struktur/40", "Ø Inhalt/50",
                "Ø Kreativität/10", "min", "max"])
    ...
    for tier in ("perfekt", "normal", "schlecht"):
        ...
        ws2.append([tier, len(sub), round(sum(sc) / len(sc), 1),
                    avg("s_structure"), avg("s_content"), avg("s_creativity"),
                    min(sc), max(sc)])
...
add_summary("Gesamt (60 Cases × 3 Eingaben)", rows)
add_summary("Nur Mittel", [r for r in rows if r["difficulty"] == "medium"])
add_summary("Nur Schwer", [r for r in rows if r["difficulty"] == "hard"])
for cat in ("market_entry", "risks_opportunities", "financial"):
    add_summary(f"Kategorie: {cat}", [r for r in rows if r["category"] == cat])

for i, w in enumerate([14, 6, 10, 13, 12, 14, 8, 8], 1):
    ws2.column_dimensions[chr(64 + i)].width = w

out = os.path.join(ROOT, sys.argv[1] if len(sys.argv) > 1 else "Creativity-Bewertungstest.xlsx")
wb.save(out)
